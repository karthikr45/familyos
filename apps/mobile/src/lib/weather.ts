...
/** Solid hero background colour by condition code + day/night (RN-friendly). */
export function heroColor(code: number, isDay: boolean): string {
  if (!isDay) return '#1e1b4b';
  const rain = [
  ...
  if (thunder.includes(code)) return '#312e81';
  if (rain.includes(code)) return '#1e40af';
  if (snow.includes(code)) return '#0ea5e9';
  if (fog.includes(code)) return '#64748b';
  if (code === 1000) return '#0284c7';
  return '#0369a1';
}
